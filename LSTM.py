...
def build_timeseries(input_data, y_col_index, time_steps, output_gap):
    ...
    for i in range(dimention_0):
        ...
        y[i] = input_data[time_steps+output_gap+i-1, y_col_index]
...
def train(input_path, output, logdir, epochs=10, learning_rate=0.0001, 
            time_steps=200, output_gap=30, batch_size=20, feature_columns=[], 
            output_index=0):
    if os.path.isfile(input_path):
        ...
        # 学習データの読み込み
        df_input = pd.read_csv(input_path, engine='python',encoding="UTF-8")
    ...
    # 学習結果を元に予測のテスト
    predict_time_series(trim_dataset(x_test_t, batch_size), trim_dataset(y_test_t, batch_size), logdir, output, time_steps, output_gap, batch_size, output_index, True)
...
def predict_time_series(x_real, y_real, logdir, input_modelfile, time_steps, output_gap, batch_size, output_index, plot_graph):
    ...
    if plot_graph:
        ...
        plt.savefig(os.path.join(logdir, 'pred_vs_real_BS'+str(batch_size)+'.png'))
        plt.close(fig)
        ...
        plt.savefig(os.path.join(logdir, 'gain_BS.png'))
        plt.close(fig)
...
def predict_all(input_csv, input_modelfile, logdir, time_steps, output_gap, batch_size, feature_columns, output_index):
    ...
    # 入力データの読み込み
    df_input = pd.read_csv(input_csv, engine='python',encoding="UTF-8")
...
def main(argv):
    ...
    if is_train:
        train(input, filename, logdir, epochs=10, learning_rate=0.0001, 
            time_steps=200, output_gap=30, batch_size=20, 
            feature_columns=["open", "high", "low", "close", "Volume"], 
            output_index=0)
    else:
        predict_next(input, filename, logdir, time_steps=20, output_gap=30, 
            batch_size=20, feature_columns=["open", "high", "low", "close", "Volume"], 
            output_index=0)
